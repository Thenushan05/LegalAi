...
export class ApiClient {
  ...
  async uploadFile(
    file: File,
    uploadType: "normal" | "confidential" = "normal"
  ...
  async getUploadStatus(fileHash: string): Promise<{ status: string }> {
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.UPLOAD_STATUS}/${fileHash}`,
      {
  ...
  async deleteFile(fileHash: string): Promise<{ message: string }> {
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.DELETE_FILE}/${fileHash}`,
      {
        method: "DELETE",
        headers: this.getHeaders(),
      }
  ...
  async guestUploadFile(file: File): Promise<UploadResponse> {
    ...
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.GUEST_UPLOAD}`,
      {
        method: "POST",
        body: formData,
      }
  ...
  async guestQA(
    ...
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.GUEST_QA}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  private findFilenameInText(text: string): string | null {
    ...
    for (const file of fileList) {
      ...
      const regex = new RegExp(
        `(^|${wordBoundary.source})${escapedName}($|${wordBoundary.source})`,
        "i"
      );
  ...
  async askQuestion(
    ...
    if (!fileHash) {
      throw new Error(
        "No file specified and no files have been uploaded yet. Please upload a file first or specify a filename in your question."
      );
    }

    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.QA}`,
      {
        method: "POST",
        headers: this.getHeaders(),
        body: JSON.stringify({
  ...
  async summarizeDocument(
    ...
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.SUMMARIZE}`,
      {
        method: "POST",
        headers: this.getHeaders(),
        body: JSON.stringify({
  ...
  async compareDocuments(
    ...
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.COMPARE}`,
      {
        method: "POST",
        headers: this.getHeaders(),
        body: JSON.stringify({
  ...
  async simplifyText(
    ...
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.SIMPLIFY}`,
      {
        method: "POST",
        headers: this.getHeaders(),
        body: JSON.stringify({
  ...
  async getUserProfile(): Promise<UserProfile> {
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.USER_PROFILE}`,
      {
        method: "GET",
        headers: this.getHeaders(),
      }
  ...
  async saveChatMessage(message: any): Promise<{ message: string }> {
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.CHAT_HISTORY}`,
      {
        method: "POST",
        headers: this.getHeaders(),
  ...
  async googleSignIn(idToken: string): Promise<GoogleSignInResponse> {
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.GOOGLE_SIGNIN}`,
      {
        method: "POST",
        headers: this.getHeaders(),
        body: JSON.stringify({
  ...
  async getChatHistory(
    ...
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.CHAT_HISTORY}?${params}`,
      {
        headers: this.getHeaders(),
      }
    );
  ...
  async submitFeedback(
    ...
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.FEEDBACK}`,
      {
        method: "POST",
        headers: this.getHeaders(),
        body: JSON.stringify({
  ...
  async triggerRetrain(): Promise<{ message: string }> {
    const response = await this._fetchWithTimeout(
      `${this.baseUrl}${API_CONFIG.ENDPOINTS.RETRAIN}`,
      {
        method: "POST",
        headers: this.getHeaders(),
      }
...
export async function checkBackendHealth(): Promise<boolean> {
  try {
    const response = await fetch(
      `${API_CONFIG.BASE_URL.replace("/api", "")}/docs`
    );
    return response.ok;
  } catch {
    return false;
